...
//SAMO: Fixes bug. P1.16 is used as CONNECT signal on prototype
#ifdef PROTOTYPE
    if(fConnect)
    IO1SET |=1<<16;
    else
...
void usbSetupInterruptHandler (void)
{
  ...
  //
  VICIntSelect &= ~VIC_IntSelect_USB;
  VICVectAddr1 = (portLONG) usbISR;
  VICVectCntl1 = VIC_VectCntl_ENABLE | VIC_Channel_USB;
  VICIntEnable = VIC_IntEnable_USB;
}
...
BOOL usbHardwareInit (void)
{
  ...
  //
  PINSEL1 = (PINSEL1 & ~PCB_PINSEL1_P023_GPIO) | PCB_PINSEL1_P023_VBUS;
  GPIO0_FIODIR &= ~GPIO_IO_P23;
  PINSEL1 = (PINSEL1 & ~PCB_PINSEL1_P031_GPIO) | PCB_PINSEL1_P031_CONNECT;
//SAMO: fixes bug in prototype
#ifdef PROTOTYPE
    IO1DIR |=1<<16;
    IO1CLR |=1<<16;
#endif
